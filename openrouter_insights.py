...
"""
        ...
        try:
            ...
            print(f"OpenRouter response for: {title[:30]}... (model: {model_used})")
            return self._parse_response(content, title)

        except Exception as e:
            self._log_ai_failure(title, e)
            return self._create_fallback_insight(article)

    def _log_ai_failure(self, article_title: str, error: Exception):
        """Log when AI generation completely fails"""
        print(f"[WARN] AI insights failed for: {article_title[:50]}...")
        print(f"[WARN] Error: {type(error).__name__}: {str(error)}")
        print(f"[INFO] Using fallback insights instead")
